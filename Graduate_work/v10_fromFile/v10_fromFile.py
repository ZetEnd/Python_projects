        ...
        while True:
            if ser.in_waiting > 0:
                ...
                if line == "WiFi Connected" and not collecting:
                    ...
                    continue
                if line == "WiFi Connected" and num == 10:
                    break
...
def plot_bar(massive, num_massive):

    mas_i = [x for x in range(num_massive)] 

    plt.bar(massive, mas_i, label='MO') #Параметр label позволяет задать название величины для легенды
    plt.xlabel('vel')
...
if __name__ == "__main__":
    ...
    plot_bar(list_of_MO[:][0])
    plot_bar(list_of_MO[:][1])
    plot_bar(list_of_MO[:][2])
