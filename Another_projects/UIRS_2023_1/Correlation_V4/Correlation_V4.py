...
if __name__ == "__main__":
    ...
    width = int(img_shape_1 * scale_percent / 100) 
    ...
    while(True): 
        ...
        for di in range(0, img_test.shape[0] - img_shape_0, 2):
            for dj in range(0, img_test.shape[1] - img_shape_1, 2):
                ...
                for i in range(0, img.shape[0]):
                    for j in range(0, img.shape[1]):
                        ...
                        corr = np.abs(img[i,j] - ri_mean)*np.abs(img_test[i+di, j+dj] - img_test[di:di+img.shape[0],dj:dj+img.shape[1]].mean())
